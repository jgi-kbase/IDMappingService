...
from http.client import (
    responses,
)  # @UnresolvedImport dunno why pydev cries here, it's stdlib
import flask
...
try:
    ...
    # tested manually
    raise ValueError(
        "Did not find git commit file at "  # pragma: no cover
        + "src/jgikbase/idmapping/gitcommit.py. "  # pragma: no cover
        + "The build may not have completed correctly."
    )  # pragma: no cover
...
def _format_error(
    ...
    errjson = {
        ...
        "callid": getattr(flask_req_global, 'req_id', None),
        "time": epoch_ms(),
    }
    if errtype:
...
def _objids_to_jsonable(oids: Set[ObjectID]):
    return sorted(
        [{"ns": o.namespace_id.id, "id": o.id} for o in oids],
    ...
    for id_ in ids:
        ...
        if not isinstance(val, str):
            raise IllegalParameterError(
                "value for key {} in input JSON is not string: {}".format(id_, val)
            )
        if not val.strip():
            raise MissingParameterError(
                "value for key {} in input JSON is whitespace only".format(id_)
            )
    return ids
...
def create_app(
    ...
    @app.route(
        "/api/v1/namespace/<namespace>/user/<authsource>/<user>", methods=["PUT"]
    )
    ...
    @app.route(
        "/api/v1/namespace/<namespace>/user/<authsource>/<user>", methods=["DELETE"]
    )
    ...
    def get_namespaces():
        ...
        return flask.jsonify(
            {
                "publicly_mappable": sorted([ns.id for ns in public]),
    ...
    def create_mapping(admin_ns, other_ns):
        ...
        for id_ in ids:
            app.config[_APP].create_mapping(
                authsource,
                token,
