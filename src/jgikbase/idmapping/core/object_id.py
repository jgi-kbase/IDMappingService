...
from jgikbase.idmapping.core.arg_check import (
    check_string,
    not_none,
    ...
    def __eq__(self, other):
        if type(self) is type(other):
            return (
                ...
                and self.is_publicly_mappable == other.is_publicly_mappable
                and self.authed_users == other.authed_users
            )
        return False
...
class ObjectID:
    ...
    def __init__(self, namespace_id: NamespaceID, data_id: str) -> None:
        ...
        not_none(namespace_id, "namespace_id")
        check_string(
            data_id, "data id", max_len=1000
        )  # should maybe check for control chars
        self.namespace_id = namespace_id
        self.id = data_id
