...
def create_deploy_cfg(mongo_port):
    ...
    cfg["idmapping"][
        "auth-source-kbase-factory-module"
...
def test_create_and_get_namespace(service_port, mongo):
    ...
    r = requests.put(
        "http://localhost:" + service_port + "/api/v1/namespace/myns",
...
def test_add_remove_user(service_port, mongo):
    ...
    with requests_mock.Mocker(real_http=True) as m:
        ...
        m.get(
            KBASE_URL + "/api/V2/users/?list=imauser",
    ...
    assert_json_error_correct(
        r.json(),
        {
            "error": {
                ...
                "message": (
                    "40000 User already exists: User kbase/imauser already "
    ...
    assert_json_error_correct(
        r.json(),
        {
            "error": {
                ...
                "message": (
                    "50000 No such user: User kbase/imauser does not "
...
def test_mapping(service_port, mongo):
    ...
    r = requests.put(
        "http://localhost:" + service_port + "/api/v1/mapping/priv/pub",
        headers={
            "Authorization": "local " + lut.token,
    ...
    # get mappings
    r = requests.get(
        "http://localhost:" + service_port + "/api/v1/mapping/pub?separate",
        headers={"Authorization": "local " + lut.token},
        data=json.dumps({"ids": ["id2", "id4", "id8"]}),
    )

    assert r.json() == {
        "id2": {"other": [{"ns": "priv", "id": "id1"}], "admin": []},
    ...
    # fail get mappings
    r = requests.get(
        "http://localhost:" + service_port + "/api/v1/mapping/plub?separate",
        headers={"Authorization": "local " + lut.token},
        data=json.dumps({"ids": ["id2", "id4", "id8"]}),
    )
    ...
    # get mappings
    r = requests.get(
        "http://localhost:" + service_port + "/api/v1/mapping/pub",
        headers={"Authorization": "local " + lut.token},
        data=json.dumps({"ids": ["id2", "id4"]}),
    )

    assert r.json() == {
        "id2": {"mappings": [{"ns": "priv", "id": "id1"}]},
        "id4": {"mappings": []},
    }

    # fail delete mappings
    r = requests.delete(
        "http://localhost:" + service_port + "/api/v1/mapping/pub/priv",
        headers={
            "Authorization": "local " + lut.token,
            "content-type": "x-www-form-urlencoded",
        },
    ...
    assert_json_error_correct(
        r.json(),
        {
            "error": {
                ...
                "message": (
                    "20000 Unauthorized: User local/lu may not administrate "
                    + "namespace pub"
                ),
            }
        },
    ...
    r = requests.put(
        "http://localhost:" + service_port + "/api/v1/mapping/priv/priv",
        headers={"Authorization": "local " + lut.token},
    ...
    # get mappings
    r = requests.get(
        "http://localhost:" + service_port + "/api/v1/mapping/priv?separate",
        headers={"Authorization": "local " + lut.token},
        data=json.dumps({"ids": ["id1", "id21", "id20"]}),
    )

    assert r.json() == {
        "id1": {"admin": [{"ns": "pub", "id": "id2"}], "other": []},
